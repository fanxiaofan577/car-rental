...
<title>${siteName!""}|管理员管理-${title!""}</title>
          ...
          <div class="col-lg-12">
            <div class="card">
              <div class="card-toolbar clearfix">
                <form class="pull-right search-bar" method="get" action="list" role="form" enctype="application/x-www-form-urlencoded">
                  <div class="input-group">
                    <div class="input-group-btn">
                      <button class="btn btn-default dropdown-toggle" id="search-btn" data-toggle="dropdown" type="button" aria-haspopup="true" aria-expanded="false">
                        登陆名称 <span class="caret"></span>
                      </button>
                      <ul class="dropdown-menu">
                        <li> <a tabindex="-1" href="javascript:void(0)" data-field="title">登陆名称</a> </li>
                      </ul>
                    </div>
                    <input type="text" class="form-control" name="nickName" placeholder="请输入登陆名称">
                    ...
                    <tbody>
                      ...
                      <tr>
                        <td style="vertical-align:middle;">
                          <label class="lyear-checkbox checkbox-primary">
                            <input type="checkbox" name="ids[]" value="${account.accountId}"><span></span>
                          </label>
                        </td>
<#--                        头像-->
                        <td style="vertical-align:middle;">
                        	<#if account.avatarUrl??>
                        		<#if account.avatarUrl?length gt 0>
                        		<img src="${account.avatarUrl}" width="60px" height="60px">
                        		<#else>
...
<#--                            性别-->
                        ...
                        <td style="vertical-align:middle;">
                          <#if account.province?? && account.province?length gt 0>
                            <font class="text-success">${account.province}</font>
                          <#else>
                            <font class="text-warning">未知</font>
                          </#if>
                        </td>
                        ...
                        <td style="vertical-align:middle;">
                          <#if account.city?? && account.city?length gt 0>
                            <font class="text-success">${account.city}</font>
                          <#else>
                            <font class="text-warning">未知</font>
                          </#if>
                        </td>
                        ...
                        <td style="vertical-align:middle;">
                          <#if account.country?? && account.country?length gt 0>
                            <font class="text-success">${account.country}</font>
                          <#else>
                            <font class="text-warning">未知</font>
                          </#if>
                        </td>
...
<#--                        状态-->
                        ...
                        <td style="vertical-align:middle;" style="vertical-align:middle;"><font class="text-success">${account.createTime?string("yyyy-MM-dd")}</font></td>
                      </tr>
                    </#list>
                    <#else>
...
function deleteReq(id,url){
	$.ajax({
		...
		success:function(data){
			if(data.code == 0){
				showSuccessMsg('管理员删除成功!',function(){
